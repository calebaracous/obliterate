...
This is the master reference file for Claude Code. Read this first, then refer to the
specific docs in `/docs/` for deeper context on each domain.
...
| Cache | Vercel KV (Upstash Redis) | Precomputed meta stats, leaderboards, freshness timestamps |
| Hosting | Vercel Pro | Edge-optimized deploys; Cron Jobs for ingestion |
| Styling | Tailwind CSS v4 | Design tokens defined in `design-system.md` |
...
├── docs/                         # Extended planning docs (this folder)
│   ├── ARCHITECTURE.md
│   ├── DATABASE.md
│   ├── INGESTION.md
│   ├── ANALYTICS.md
...
│   ├── db.ts                     # DB row types (generated by introspection)
...
- See `vercel.json` and `docs/INGESTION.md` for the full cron schedule.
...
# Seed item dictionary from ao-bin-dumps
npm run seed:items
...
| File | What it covers |
|---|---|
| `docs/ARCHITECTURE.md` | System design, data flow diagrams, caching strategy |
| `docs/DATABASE.md` | Full schema, indexes, migration notes |
| `docs/INGESTION.md` | Gameinfo API details, polling logic, cron schedule |
| `docs/ANALYTICS.md` | Win-rate computation, matchup matrix, meta scoring |
| `docs/CLASSIFIER.md` | Content-type inference algorithm |
| `docs/API.md` | All internal API route specs |
