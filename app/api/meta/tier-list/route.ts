...
import { computeWeaponWinRates } from '@/lib/analytics/win-rate'
import { computeTierList } from '@/lib/analytics/tier-list'
import { kvGet, kvSet, kvCacheKey } from '@/lib/cache/kv'

type TierList = ReturnType<typeof computeTierList>
...
export async function GET(request: NextRequest) {
  ...
  const p = parsed.data
  const cacheKey = kvCacheKey('meta', 'tier-list', p.region, p.contentType, p.ipBracket)

  try {
    const cached = await kvGet<TierList>(cacheKey)
    if (cached) {
      return ok(
        { tiers: cached, snapshotDate: new Date().toISOString().split('T')[0] },
        { cached: true },
      )
    }
  } catch {
  ...
  try {
    ...
    const tiers = computeTierList(weapons)

    try {
      await kvSet(cacheKey, tiers, 3600)
    } catch {
      // non-fatal
    }

    return ok(
      { tiers, snapshotDate: new Date().toISOString().split('T')[0] },
      { cached: false },
    )
  } catch {
